...
clients<- read.csv("lab_02/clients.csv")
...
median_year <- median(clients$Year_Birth, na.rm = TRUE)
# c) Bạn sử dụng đoạn mã nào để điền các giá trị bị thiếu của Year_Birth (nếu có)?
clients$Year_Birth[is.na(clients$Year_Birth)] <- median_year
...
cols_to_factor <- c("Education", "Response", "Complain")
clients[cols_to_factor] <- lapply(clients[cols_to_factor], as.factor)
...
save(clients, file = "clientInR.RData")
